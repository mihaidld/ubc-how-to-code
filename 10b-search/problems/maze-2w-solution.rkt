...
(define (solvable? m)  
  (local [(define (solve/one p)                   
            (cond [(solved? p m) true]          ;trivial case: solved produce maze is solvable
                  [else
                   (solve/lop (next-ps m p))])) ;generative recursion solve next positions
    ...
    (solve/one (make-pos 0 0))))    ;trampoline with position (0 0)
...
(define (solved? p m)
  (= (pos-x p) (pos-y p) (sub1 (sqrt (length m)))))     ;solved if x = y  = square root of length - 1 (since 0-indexed) e.g. (4 4) in maze length 25
...
(define (next-ps m p)
  (local [(define SIZE (sqrt (length m)))     ;constant storing each side lenghth (number of rows / columns)
          (define (new-ps p)                  ;produce list of 2 positions where either x oy y incremented by 1
            (local [(define x (pos-x p))
                    (define y (pos-y p))]
              (list (make-pos (add1 x)      y)
                    (make-pos       x (add1 y)))))
          
          (define (valid-only m lop)         ;filter only valid positions
            (filter valid? lop))
          
          (define (valid? p)                 ;valid means x and y between 0 and SIZE - 1 and value in maze at that position is true (not wall)
            (and (<= 0 (pos-x p) (- SIZE 1))
                 (<= 0 (pos-y p) (- SIZE 1))
                 (mref m p)))]
    
    (valid-only m (new-ps p))))    ;filter only valid list of positions
...
(define (render m)
  (local [(define SIZE (sqrt (length m)))          ;for 4x4 this is 4
          ...
          (define (cv->image cv)                   ;current value to image: if value is true produce gray cell, otherwise black cell
            (cond [cv   OPEN]
                  [else WALL]))]
    
    (foldr (lambda (i img)                         ;combination function for foldr is lambda function which must take 2 args (i = row number = first element of the list IDXS) and image resulted of NR of calling foldr on rest of list
             (above (foldr (lambda (j img)         ;place first layer above NR on rest of list (rest of layers) 
                             (beside (cv->image (list-ref m (+ (* i SIZE) j)));to form a layer place beside square images of different colours
                                     img))
                           empty-image
                           IDXS)
                    img))
           empty-image                             ;base case of fold is empty-image
           IDXS)))                                 ;list for foldr is indices of rows/columns IDXS
